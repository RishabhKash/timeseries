...
def train_predict(
    ...
    data.loc[data[project] <= 0, project] = np.nan
    data[project] = data[project].interpolate(limit_direction="both")
...
def train_predict_centre(
        processed_data: pd.DataFrame,
        project: str
):
    """
    Tests an array of time series algorithm for each centre
    and predicts for the next six months using the best performing model
    ...
    temp = processed_data.copy()
    temp.iloc[:, 0] = pd.to_datetime(temp.iloc[:, 0])
    temp.iloc[:, 0] = temp.iloc[:, 0].apply(lambda x: x + relativedelta(day=31))
    start_date = datetime.now().strftime("%Y-%m")
    end_date = (datetime.now() + relativedelta(months=5)).strftime("%Y-%m")
    model_performance = pd.DataFrame()
    centre_wise_predictions = pd.DataFrame()

    # creating location wise tabular data
    for locations in temp.iloc[:, 1].unique():
        data = temp[temp.iloc[:, 1] == locations].drop(temp.columns[1], axis=1).sort_values(by=temp.columns[0])
        data.loc[data[project] <= 0, project] = np.nan
        data[project] = data[project].interpolate(limit_direction="both")
        data = data.set_index(data.columns[0])
        ...
        train_len = len(data) - test_len
        performance = pd.DataFrame()
        logging.info(f"{locations} train length {train_len} test length {test_len}")

        # training different time series models
        outcome = ideal_prophet(data=data, train_len=train_len, test_len=test_len, project=project)
        outcome['centre'] = locations
        performance = pd.concat([performance, outcome])
        ...
        if performance.iloc[0][1] in ('simple_exponential_smoothing', 'exponential_smoothing_add',
            ...
            if performance.iloc[0][5] > 0:
                ...
                result = pd.DataFrame(result)
                result.columns = [locations]
                result[locations] = result[locations].cumsum()
                count = 0
                while count < performance.iloc[0][5]:
                    result[locations] = result[locations].add(boxcox_data[count])
                    count += 1
            ...
            result[locations] = inv_boxcox(result[locations], lmbda)
            result = result[-6:]
        ...
        result = round(result, 0).abs()
        centre_wise_predictions = pd.concat([centre_wise_predictions, result], axis=1)

    return model_performance, centre_wise_predictions.T
